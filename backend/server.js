...
const DISCORD_WEBHOOK_CHAT = "BURAYA_SOHBET_WEBHOOK_URLSINI_YAZ"; // Sohbet için webhook URL'ini buraya yaz
...
app.post('/api/calls', async (req, res) => {
  const call = { 
    ...
    timestamp: new Date().toLocaleString('tr-TR'), 
    status: 'ALINDI',
    createdBy: req.body.createdBy || 'Bilinmeyen', // Çağrıyı oluşturan kullanıcı
    ...req.body 
  };
  calls.push(call);
  ...
  try {
    ...
    // Acil çağrıda iletişim bilgisi ekle
    if (isEmergency) {
      ...
      // Bot'a DM göndermesi için istek at
      try {
        await fetch('http://localhost:3001/send-emergency-dm', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
        ...
        console.log('✅ Acil DM istekleri gönderildi');
      } catch (dmErr) {
        console.log('⚠️ DM gönderilemedi:', dmErr.message);
      }
    }
    ...
    console.log(`Discord bildirimi gönderildi - Çağrı #${call.id}${isEmergency ? ' (ACİL)' : ''} - Yapan: ${call.createdBy}`);
  } catch (err) {
...
const PORT = 3000;
server.listen(PORT, () => console.log(`🚔 OCST Backend → http://localhost:${PORT}`));